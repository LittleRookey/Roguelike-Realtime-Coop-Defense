...
public class LoadingFadeEffect : SingletonPersistent<LoadingFadeEffect>
{
    ...
    [SerializeField]
    RawImage m_loadingBackground;          // The background image to change alpha for effect

    [SerializeField]
    [Range(0f, 5f)]
    float m_loadingTime;            // A range of time to wait for every repetition on the effect
    ...
    async void FadeAllEffect()
    {
        // Do the fade in effect 
        FadeInEffect();

        // Wait a x time to call
        await Task.Delay((int)(m_loadingTime * 1000)*2);
    ...
    void FadeInEffect()
    {
        ...
        m_loadingBackground.DOFade(255f, m_loadingTime)
            .OnComplete(() => {
                s_canLoad = true; 
            });
    ...
    void FadeOutEffect()
    {
        ...
        m_loadingBackground.DOFade(0f, m_loadingTime)
           .OnComplete(() => { m_loadingBackground.gameObject.SetActive(false); });
