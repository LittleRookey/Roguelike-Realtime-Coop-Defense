...
public class NetworkObjectPool : NetworkBehaviour
{
    ...
    public NetworkObject GetNetworkObject(GameObject prefab, Vector3 position, Quaternion rotation, Transform parent)
    {
        return GetNetworkObjectInternal(prefab, position, rotation, parent);
    }
    ...
    /// </summary>
    private void RegisterPrefabInternal(GameObject prefab, int prewarmCount, Transform parent=null)
    {
        prefabs.Add(prefab);
        ...
        {
            var go = CreateInstance(prefab, parent);
    ...
    {
        if (parent == null) return Instantiate(prefab);
    ...
    /// <returns></returns>
    private NetworkObject GetNetworkObjectInternal(GameObject prefab, Vector3 position, Quaternion rotation, Transform parent)
    {
        var queue = pooledObjects[prefab];
        ...
        {
            networkObject = CreateInstance(prefab, parent).GetComponent<NetworkObject>();
        }
